...
//! Tests for the [`format!`](multitude::format) macro.
...
fn format_macro_returns_arena_string() {
    ...
    let arena = Arena::new();
    let mut s = multitude::builders::format!(in &arena, "Hello, {}!", "Alice");
    s.push_str(" extended");
...
fn format_macro_with_multiple_args() {
    let arena = Arena::new();
    let s = multitude::builders::format!(in &arena, "{}+{}={}", 2, 3, 5);
    assert_eq!(&*s, "2+3=5");
}
...
fn format_macro_empty_format_string() {
    let arena = Arena::new();
    let s = multitude::builders::format!(in &arena, "");
    assert_eq!(&*s, "");
}
